...
public class GetTripQueryHandler : IRequestHandler<GetTrip, Maybe<Trip>>, IRequestHandler<GetTripsByCountry, ICollection<Trip>>, IRequestHandler<GetTrips, ICollection<Trip>>
{
    private readonly IRegistrationsContext _registrationsContext;
    
    public GetTripQueryHandler(IRegistrationsContext registrationsContext)
    {
    ...
    {
        var res = await _registrationsContext.Trips.FindAsync(new object?[] { request.Id }, cancellationToken);
